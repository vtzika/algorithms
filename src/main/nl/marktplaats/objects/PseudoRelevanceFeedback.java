...
public class PseudoRelevanceFeedback {
	...
	public String findBiggestCTRAndCreateNewQueries(List<Integer> cat_ids) throws Exception {
		...
		{	
			...
			List<Long> maxCTRAds = hashMan.getMaxCTRFromAds(sql.selectHashMapLongDoubbleQuery("select id,CTR from allL2AndCTR where l2="+id,"algorithms"));
			sql.insertQuery("insert into topCTRL2 values ("+id+","+maxCTRAds.get(0)+","+maxCTRAds.get(1)+","+maxCTRAds.get(2)+","+maxCTRAds.get(3)+","+maxCTRAds.get(4)+");","algorithms");
