...
public abstract class ExtendQuery {
	...
	public void saveResults(Configuration configuration) throws Exception {
		...
		sql.insertQuery("insert into "+configuration.getInputTable()+" values("+this.getQuery().getQID()+",'"+this.getExtendedQuery().getQString()+"','"+configuration.getAobMethod()+"');", configuration.getDb());
	}
